...
/* =========================================================
   Registro de login por RPC
========================================================= */
async function registerLoginEventOnce(params?: {
  source?: string;
  pathname?: string;
}) {
  try {
    ...
    const { data, error } = await supabase.rpc("register_login_event", {
      p_source: params?.source ?? "first-login-page",
      p_metadata: {
        pathname: params?.pathname ?? window.location.pathname,
    ...
    if (error) {
      console.error("MQA register_login_event RPC error:", error);
      return {
        ok: false,
        skipped: false,
    ...
    console.error("MQA registerLoginEventOnce unexpected error:", err);
    return {
      ok: false,
      skipped: false,
